...
private fun EntryScreen(
  ...
  val onToggleEdit: (Boolean) -> Unit = {
    setEditing(it)
    if (it) onEdit(day) else onEdit(null)
  }
  val backAction: () -> Unit = { if (editing) onToggleEdit(false) else onBack() }
  ...
  val onLocationChange: (Location) -> Unit = { loc ->
    editingCopy?.let { onEdit(it.copy(location = loc)) }
  }
  val tags =
      if (editing && editingCopy != null) {
        editingCopy.tags
  ...
  Scaffold(
      ...
      topBar = {
        ...
        Column(modifier = Modifier.padding(paddingValues)) {
          ...
          SecondaryTabRow(selectedTabIndex = tabState) {
            EntrySection.values().forEachIndexed { idx, sect ->
              Tab(
                  selected = idx == tabState,
                  onClick = {
                  ...
                  icon = { Icon(imageVector = sect.icon, contentDescription = sect.label) },
                  text = { Text(text = sect.label) })
            }
          }
          //          EntrySectionToggle(selectedProvider = { selectedSection }, onSelect =
          // setSelectedSection)
          HorizontalPager(state = pagerState, modifier = Modifier.fillMaxSize()) { page ->
            Column(modifier = Modifier.fillMaxSize().padding(top = 16.dp), verticalArrangement = Arrangement.Top) {
              when (page) {
                0 ->
                  EntrySummarySection(
                    editing = editing, text = summaryText, onTextChange = onSummaryChange)
                1 ->
...
private fun NotFoundScreen(dayId: Long, onBack: () -> Unit) {
  ...
  Scaffold(
      modifier = Modifier.semantics { testTag = tt },
      topBar = { TopAppBar(title = {}, navigationIcon = { BackButton(onBack = onBack) }) }) {
        Column(
...
enum class EntrySection(
  ...
  Tags(label = "Tags", icon = Icons.Filled.Tag, descriptionResId = R.string.single_tags)
}
